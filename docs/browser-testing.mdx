...
<Info>
On macOS and Linux, dev servers keep running if Vibe Kanban restarts (for example during an update). The new instance reconnects to the running process and its logs automatically, so you don't need to start it again. On Windows, dev servers are restarted automatically when Vibe Kanban starts.
</Info>
