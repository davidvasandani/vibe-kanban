...
| `create_issue` | Create a new issue | `title` | `project_id`<br/>`description`<br/>`priority`<br/>`parent_issue_id`<br/>`pipeline_ids`<br/>`pipeline_stage_ids`<br/>`executor` | Created issue ID |
...
`create_issue`'s `pipeline_ids`, `pipeline_stage_ids`, and `executor` parameters compose the same `## Pipeline` block that the New Issue UI appends to a task's description. Call `list_pipelines` first to find available pipeline and stage IDs; see [Pipelines](#pipelines) below.
...
| Tool | Purpose | Required Parameters | Optional Parameters | Returns |
|------|---------|-------------------|-------------------|---------|
| `list_pipelines` | List the available task pipelines (`~/.vibe-kanban/pipelines/*.toml`) | None | None | List of pipelines with their ordered stages |

Each pipeline stage has an `id`, `label`, `prompt_fragment`, `default_enabled` flag, and `heavy` flag. Pass a pipeline's `id` in `create_issue`'s `pipeline_ids`, and (optionally) a subset of its stage `id`s in `pipeline_stage_ids` — omitting `pipeline_stage_ids` enables each selected pipeline's `default_enabled` stages.
...
| Tool | Purpose | Required Parameters | Optional Parameters | Returns |
|------|---------|-------------------|-------------------|---------|
| `start_workspace` | Create a workspace and start its first coding-agent session | `name`<br/>`executor`<br/>`repositories` | `prompt`<br/>`variant`<br/>`model_id`<br/>`agent_id`<br/>`reasoning_id`<br/>`issue_id` | Workspace ID |
...
When an agent's turn ends, Vibe Kanban stops the agent's entire process group, so any process the agent backgrounds with `&` dies with the turn. Background helpers are the sanctioned way to keep a long-lived process (file watcher, tunnel, log follower) running after the turn ends. Do not use `setsid` or `nohup` workarounds — they leave processes that Vibe Kanban cannot display, stop, or clean up.

| Tool | Purpose | Required Parameters | Optional Parameters | Returns |
|------|---------|-------------------|-------------------|---------|
| `spawn_background_helper` | Spawn a tracked helper process that survives the end of the agent turn | `script` | `working_dir`<br/>`workspace_id` | Execution process ID |
| `list_background_helpers` | List running background helpers for a workspace | None | `workspace_id` | List of helpers with scripts and status |
| `stop_background_helper` | Stop a running background helper | `execution_process_id` | None | Stop confirmation |

Helpers are tracked as execution processes: they appear in the workspace's Processes tab, keep running across Vibe Kanban restarts (like dev servers), and are stopped when the workspace is archived. Each workspace can run at most 5 helpers at a time. The `working_dir` is relative to the workspace root and must not contain `..`.
...
<AccordionGroup>
<Accordion title="A coding agent doesn't see the Vibe Kanban MCP server">
The **MCP Servers** settings page is per-agent: when you save, Vibe Kanban writes the configuration to the selected agent's own config file, and to that file only.

If your workspaces launch a different agent from the one you had selected when saving, that agent never receives the configuration.

<Steps>
<Step title="Confirm which agent your workspaces use">
  Check the executor configured for the workspaces where the server is missing.
</Step>

<Step title="Save the server for that agent">
  Open **Settings → MCP Servers**, switch the agent selector to match, click the **Vibe Kanban** card under **Popular servers**, then click **Save Settings**.

  <Check>
  The `vibe_kanban` entry now appears in that agent's MCP configuration file (for example, `~/.claude.json` for Claude Code).
  </Check>
</Step>

<Step title="Restart the agent session">
  Coding agents read MCP configuration at startup, so start a new session to pick up the change.
</Step>
</Steps>

Repeat these steps for each agent you use across your workspaces.
</Accordion>

<Accordion title="The MCP server is only available intermittently">
By default the MCP server discovers the Vibe Kanban backend through a temporary port file that the backend writes while it runs. If the backend restarts, that file is briefly rewritten, and the server may fail to connect for sessions that start during the restart.

Pin the backend address to make discovery deterministic. Add a `VIBE_BACKEND_URL` environment variable to the server entry, pointing at the port your Vibe Kanban backend listens on:
...
Restart the agent session afterwards so the new environment takes effect.

<Warning>
This is reliable only when the backend always listens on the same port. Set a fixed `BACKEND_PORT` in your `.env` so the address in `VIBE_BACKEND_URL` stays valid across restarts. Saving from the **MCP Servers** settings page rewrites the entry and removes the `env` block, so re-add it after any save from the UI.
</Warning>
</Accordion>
</AccordionGroup>
