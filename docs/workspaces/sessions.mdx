...
If Vibe Kanban restarts while an agent is running — for example during an update — the run is marked as **interrupted** rather than failed:

- Any uncommitted changes from the run are preserved as a work-in-progress commit on the workspace branch
- The chat shows a banner offering to **Resume** the run; one click sends a follow-up that continues the agent from where it left off

<Info>
The agent's session history is preserved across the restart, so resuming continues the same conversation rather than starting over.
</Info>
