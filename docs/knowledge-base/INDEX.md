...
| [shared-mcp-configuration](shared-mcp-configuration.md) | Shared MCP definitions are derived from native configs and written per executor; authenticated HTTP servers can use a loopback gateway with encrypted central tokens, refresh/revocation, streamed forwarding, local capability control, safe redaction, and stable reconnects | `a898-allow-mcp-server`, `4ae2-add-a-shared-mcp` |
