...
- [x] T001 Add typed refresh statuses, safe errors, server snapshots, and
  executor-control abstractions in
  `crates/executors/src/mcp_refresh.rs` and
  `crates/executors/src/lib.rs`.
- [x] T002 Add the process-local per-session claim/confirm state machine,
...
- [x] T003 [P] Add safe allow-listed error mapping/redaction tests in
...
- [x] T005 Extend `SpawnedChild` with the optional live MCP control handoff and
...
- [x] T006 Register/remove live controls, store refresh state, implement
...
- [x] T009 Register refresh types and regenerate checked-in TypeScript/schema
  outputs via `crates/server/src/bin/generate_types.rs`, `shared/types.ts`, and
  `shared/schemas/`. Depends on T001, T008.
- [x] T010 [P] Add `refresh_mcp_tools` request/response client support in
  `crates/mcp/src/task_server/tools/mod.rs` and
  `crates/mcp/src/task_server/tools/sessions.rs`. Depends on T008.
- [x] T011 Add the scoped/global VK MCP tool, router membership tests, and safe
...
- [x] T012 Add refresh/status API client methods in
...
- [x] T014 Add the active-session refresh control, pending polling, confirmed
...
- [x] T019 [P] Document the supported-executor contract, pending-next-turn
  semantics, statuses, and remediation in
  `docs/integrations/mcp-server-configuration.mdx`. Depends on T014.

## Layer 6 — Verification

- [x] T020 Install locked frontend dependencies with
  `pnpm install --frozen-lockfile`, then run focused Rust and frontend tests.
  Depends on T007, T011, T015, T016, T017.
- [x] T021 [P] Run generated-type/schema checks and relevant TypeScript checks.
  Depends on T009, T015.
- [x] T022 [P] Run relevant Rust workspace checks/tests. Depends on T020.
- [x] T023 Run `pnpm run format`, rerun affected validation, and inspect the diff
  for unrelated/generated changes. Depends on T019, T020, T021, T022.

## Layer 7 — Independent review and knowledge

- [x] T024 Run independent Codex diff review, address confirmed significant
  findings, rerun relevant validation, and repeat until clean. Depends on T023.
- [x] T025 Update `docs/knowledge-base/` and its index with reusable live MCP
