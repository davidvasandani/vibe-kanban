...
- [ ] T001 [P] In `crates/executors/src/executors/claude.rs`, insert
...
- [ ] T002 [P] In `crates/executors/src/executors/cursor.rs`, make four
  coordinated edits:
...
- [ ] T003 [P] In `crates/executors/src/executors/copilot.rs`, insert
  `("claude-opus-5", "Claude Opus 5")` before `("claude-opus-4.8", ...)`
  in `discover_options()` (line ~201).

- [ ] T004 [P] In `crates/executors/src/executors/droid.rs`, make two edits:
...
- [ ] T005 [P] Add a unit test in `claude.rs` `mod tests` (after line ~2861)
...
- [ ] T006 [P] Add tests in `cursor.rs` `mod tests` (after line ~1404):
...
- [ ] T007 [P] Add a `#[cfg(test)] mod tests` block in `copilot.rs` with a
  test verifying `discover_options()` output contains `"claude-opus-5"`.
  Depends on T003.

- [ ] T008 [P] Add a `#[cfg(test)] mod tests` block in `droid.rs` with a
  test verifying `discover_options()` output contains `"claude-opus-5"`.
  Depends on T004.
...
- [ ] T009 Run `pnpm run generate-types` to regenerate `shared/schemas/*.json`
...
- [ ] T010 [P] Run `cargo test --workspace` to execute all focused tests and
  confirm no regressions. Depends on T005, T006, T007, T008, T009.

- [ ] T011 [P] Run `pnpm run generate-types:check` to confirm generated
  artifacts are in sync with source. Depends on T009.

- [ ] T012 Run `pnpm run format` to format all Rust and web code. Depends on
  T010, T011.

- [ ] T013 Run `pnpm run lint` (ESLint + cargo clippy) and `pnpm run check`
  (frontend type checks + backend Rust checks). Depends on T012.

## Layer 5 — Independent review

- [ ] T014 Run an independent Codex review of the complete diff
...
- [ ] T015 Create or update `docs/knowledge-base/executor-model-catalog-maintenance.md`
