...
- [x] T001 Audit the restart launch chain and record whether Codex starts a new
...
- [x] T002 Audit the internal live-refresh chain and status claims in
...
- [x] T003 [P] Audit MCP management/restart wording and status sources in
...
- [x] T004 Add a deterministic Codex/app-server inventory fixture that exposes
...
- [x] T005 Add stdio addition, removal, and same-name input-schema replacement
...
- [x] T006 [P] Add a streamable-HTTP materialization regression in
  `crates/executors/src/shared_mcp_config.rs` (depends on T001).
- [x] T007 [P] Extend restart handoff tests for latest-config/fresh-process
  semantics in
...
- [x] T008 Implement the smallest correction required to ensure a fresh agent
...
- [x] T009 Correct any false-success or stale-snapshot behavior found in the
...
- [x] T010 Correct only demonstrated Vibe Kanban status/wording conflation in
  `packages/web-core/src/shared/dialogs/settings/settings/McpSettingsSection.tsx`,
  `packages/web-core/src/features/workspace-chat/ui/SessionChatBoxContainer.tsx`,
...
- [x] T011 Run locked dependency setup, focused Rust/web tests, type generation
