...
- [x] T001 Bring the task branch forward to the VAS-356-capable `origin/main`
      baseline, preserving task documents and checking the resulting diff.
- [x] T002 Add authenticated refresh request/outcome types and fixtures in
      `crates/cluster-protocol/src/lib.rs` (depends on T001).
- [x] T003 [P] Add explicit secret-safe materialization and reload/bootstrap
      categories plus tests in `crates/executors/src/mcp_refresh.rs` (depends on
      T001).

## Layer 2: Worker refresh ownership

- [x] T004 Refactor scoped MCP preparation and live `WorkerJob` state in
      ...
      T002, T003).
- [x] T005 Implement worker-side validate -> atomic rematerialize -> Codex
      reload ordering and phase outcomes in `crates/worker/src/execution.rs`
      (depends on T004).
- [x] T006 Wire the signed execution refresh route and safe error mapping in
      `crates/worker/src/worker_api.rs` (depends on T002, T005).
- [x] T007 [P] Add the matching signed client operation and response decoding in
      `crates/services/src/services/cluster/client.rs` (depends on T002).

## Layer 3: Coordinator routing and result semantics

- [x] T008 Extract one authoritative Codex profile/settings snapshot resolver
      for dispatch and refresh in `crates/local-deployment/src/container.rs`
      (depends on T001).
- [x] T009 Route remote refresh by persisted execution/worker affinity and map
...
- [x] T010 Regenerate Rust-to-TypeScript contracts with the repository generator,
...
- [x] T017 Reconcile `SPEC.md`, `IMPLEMENTATION_PLAN.md`, and pipeline artifacts
      with the implemented behavior and final file set.
- [x] T018 Run independent `codex review --uncommitted`, address every confirmed
      significant finding, and repeat focused verification/review until clean.
- [x] T019 Distill reusable worker-scoped live-refresh knowledge into
