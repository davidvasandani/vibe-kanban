...
- **INFO — `spec.md` / `clarifications.md`:** All three original presentation
  ...
  on an always-visible compact header item, server-owned age, minute refresh,
...
- **WARNING — `plan.md` mixed-version risk:** The generated Rust contract makes
  `started_at` required, but a browser can temporarily receive an older response
  with no field. Controller/runtime normalization must explicitly coerce absent
  or invalid values to `null`; compile-time typing alone does not cover this.
