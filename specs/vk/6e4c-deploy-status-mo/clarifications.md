...
| What timestamp defines deployment? | The immutable release build timestamp, serialized by the existing system-information response and release manifest. | A browser timestamp resets on reload and commit time can predate deployment; one build timestamp identifies the release consistently across runtime and deployment metadata. |
