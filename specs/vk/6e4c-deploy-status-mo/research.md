...
Chosen: a UTC timestamp fixed when the server config router is constructed and
returned in `/api/info`. It is stable across browser reloads and requires no
storage or infrastructure change.
