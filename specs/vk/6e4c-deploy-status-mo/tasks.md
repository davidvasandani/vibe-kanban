...
- [x] T001 Add a stable process/router start timestamp and additive
      `UserSystemInfo.started_at` field in
      `crates/server/src/routes/config.rs`.
- [x] T002 Add or update focused Rust coverage for timestamp stability and
      serialization in `crates/server/src/routes/config.rs` (depends on T001).
...
- [x] T015 Distill reusable shipped knowledge into
      ...
      (depends on T014).
- [ ] T016 Commit the completed feature and merge branch
