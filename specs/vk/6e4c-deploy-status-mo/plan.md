...
1. In `crates/server/src/routes/config.rs`, capture a process-scoped UTC start
   timestamp when the config router is constructed and expose it as an additive
   `started_at` field on `UserSystemInfo`. Router construction happens during
   server startup and yields one stable timestamp for the process lifetime.
2. Regenerate `shared/types.ts` so `UserSystemInfo.started_at` is consumed from
...
- IV, Shared boundaries: `web-core` owns system data; `packages/ui` owns navbar
  presentation.
- XXII, Responsive operational identity: mobile consumes the same SHA plus an
  authoritative stable server start timestamp.
...
- A process-start timestamp is an application deployment approximation: it
  resets if the service restarts without a new release, which truthfully
  describes the lifetime of the currently running deployment process.
