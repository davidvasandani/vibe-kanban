...
/**
 * Delimiters bounding the generated `## Pipeline` block inside a task
 ...
 * stream) and persists onto the workspace so the task can render live
...
const EXECUTOR_LINE_RE =
  /^- Run this task with the \*\*.+\*\* execution agent:/;
...
/**
 * Compose the directive line that pins the task to a specific execution agent.
...
export function composeExecutorLine(
  ...
  if (!trimmed) return '';
  return `- Run this task with the **${trimmed}** execution agent: pass \`executor: "${trimmed}"\` when starting the workspace.`;
}
 ...
 * delimiters are absent (e.g. an older task, or one whose block was hand-edited
 * past the delimiters).
 */
 ...
 * out of a task's `## Pipeline` block, for rendering live "stage N of M"
