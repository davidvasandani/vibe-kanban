//! Detects `VK-PIPELINE-STAGE: <n>` markers emitted by a task's execution
...
//!
//! Executor child processes funnel their raw output into the per-execution
//! `MsgStore` as `LogMsg::Stdout` (see `track_child_msgs_in_store` in
//! `crates/local-deployment/src/container.rs`), so a single regex scan over
//! that shared stream detects the marker with no per-executor knowledge. The
//! tracker is spawned from `ContainerService::start_execution` for
//! coding-agent runs.
